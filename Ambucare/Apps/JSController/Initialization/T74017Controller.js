...
app.controller("T74017Controller", ["$scope", "$filter", "$http", "$window", "T74017Service", "uiGridConstants", "DTOptionsBuilder", "DTColumnBuilder", "Data","LabelService",
    function ($scope, $filter, $http, $window, T74017Service, uiGridConstants, DTOptionsBuilder, DTColumnBuilder, Data, LabelService) {

        //For Gried Data
        $scope.obj = {};
        $scope.obj =Data;
        var Label = T74017Service.getLabel();
        Label.then(function (results) {
            $scope.LabelList = results;
        });

        //For Entry User
        var EntryUser = T74017Service.EntryUser();
        ...
        var labelData = LabelService.getlabeldata('T74017');
        ...
        $scope.Save_Click = function () {
            if ($scope.obj.T_LANG2_NAME != undefined) {
                if ($scope.obj.T_AMBU_OWN_TYPE_ID != undefined) {

                    var update = T74017Service.InsertOrUpdate($scope.obj);
                    ...
                    var save = T74017Service.InsertOrUpdate($scope.obj);
        ...
        $scope.Clear = function () {
            $scope.obj.T_AMBU_OWN_TYPE_ID = '';
            $scope.obj.T_LANG1_NAME = '';
            $scope.obj.T_LANG2_NAME = undefined;

            $scope.obj.T_AMBU_OWN_TYPE_ID = undefined;
        }
        //For Delete
        $scope.Delete_Click = function () {

            if ($scope.obj.T_AMBU_OWN_TYPE_ID != null) {
                if ($window.confirm('Are you sure to want delete ?')) {
                    var dele = T74017Service.deleteData($scope.obj.T_AMBU_OWN_TYPE_ID);
        ...
        $scope.dtColumns = [
            ...
            DTColumnBuilder.newColumn("RowNumber", "Id").withOption('name', 'T_AMBU_OWN_TYPE_ID').notVisible(),
        ...
        function someClickHandler(info) {
            $scope.obj.T_AMBU_OWN_TYPE_ID = info.T_AMBU_OWN_TYPE_ID;
