...
app.controller("T74022Controller", ["$scope", "$filter", "$http", "$window", "T74022Service", "uiGridConstants", "DTOptionsBuilder", "DTColumnBuilder", "Data","LabelService",
    function ($scope, $filter, $http, $window, T74022Service, uiGridConstants, DTOptionsBuilder, DTColumnBuilder, Data, LabelService) {

        //For Gried Data
        $scope.obj = {};
         $scope.obj = Data;
        var Label = T74022Service.getLabel();
        Label.then(function (results) {
            $scope.LabelList = results;
        });

        //For Entry User
        var EntryUser = T74022Service.EntryUser();
        ...
        var labelData = LabelService.getlabeldata('T74022');
        ...
        $scope.Save_Click = function () {
            if ($scope.obj.T_LANG2_NAME != undefined) {
                if ($scope.obj.T_DOC_DEGREE_ID != undefined) {

                    var update = T74022Service.InsertOrUpdate($scope.obj);
                    ...
                    var save = T74022Service.InsertOrUpdate($scope.obj);
        ...
        $scope.Clear=function() {
            ...
            $scope.obj.T_DOC_DEGREE_ID = undefined;
        }
        //For Delete
        $scope.Delete_Click = function () {

            if ($scope.obj.T_DOC_DEGREE_ID != null) {
                if ($window.confirm('Are you sure to want delete ?')) {
                    var dele = T74022Service.deleteData($scope.obj.T_DOC_DEGREE_ID);
        ...
        $scope.dtColumns = [
            ...
            DTColumnBuilder.newColumn("RowNumber", "Id").withOption('name', 'T_DOC_DEGREE_ID').notVisible(),
        ...
        function someClickHandler(info) {
            $scope.obj.T_DOC_DEGREE_ID = info.T_DOC_DEGREE_ID;
