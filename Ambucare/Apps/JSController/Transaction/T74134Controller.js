...
app.controller("T74134Controller", ["$scope", "$window", "T74134Service","LabelService", "Data", "$filter","$rootScope",
    function ($scope, $window, T74134Service,LabelService, Data, $filter, $rootScope) {
       ...
       // $scope.obj.HidenfieldService = 'MEDICAL';
                    ...
                    item.then(function (i) {
                      ...
                      //  $scope.obj.ItemList = jsonItem;
                        for (var j = 0; j < jsonItem.length; j++) {
                            ...
                            var docSugg = jsonItem[j].T_DOSE_DURATION_CODE == null ? 0 : jsonItem[j].T_DOSE_DURATION_CODE;
                            var admi = jsonItem[j].ADMINISTERED_QUTY == null ? 0 : jsonItem[j].ADMINISTERED_QUTY;

                            if (docSugg === 0 && admi === 0) {
                                $scope.masterObj.ISSU_QTY = 0;
                            }
                            else if (docSugg > admi) {
                                $scope.masterObj.ISSU_QTY = docSugg - admi ;
                            }
                            else {
                                $scope.masterObj.ISSU_QTY = 0;
                            }
        ...
        var customEvent34=$rootScope.$on('T74134Emit', function (event, data) {
            ...
            if (data == 'm') {
                ...
                var save = T74134Service.saveData($scope.t74036, $scope.t74037);
                    ...
                    alert('Please Check Medicine first');
                }
            }
        ...
        //$rootScope.$on('T74134Emits', function (event, data) {
        //    if (data == 's') {
        //        $scope.t74074 = {};
        //        $scope.t74079 = {};
        //        $scope.t74079 = [];
        ...
        function loadItem() {
            ...
            item.then(function (i) {
                ...
                for (var j = 0; j < jsonItem.length; j++) {
                    ...
                    //else if (admi > docSugg) {
                    //    $scope.masterObj.ISSU_QTY = docSgg_admi - admi;
                    //}
                    else {
                        $scope.masterObj.ISSU_QTY = 0;
                    }
                    ...
                    //if (jsonItem[j].ADMINISTERED_QUTY !== null && jsonItem[j].T_DOSE_DURATION_CODE !== null) {
                    //    $scope.masterObj.ISSU_QTY = 0;
                    //}
                    //else if (jsonItem[j].T_DOSE_DURATION_CODE !== null) {
                    //    $scope.masterObj.ISSU_QTY = jsonItem[j].T_DOSE_DURATION_CODE;
                    //}
                    //else {
                    //    $scope.masterObj.ISSU_QTY = 0;
                    //}
        ...
        $scope.CheckForQuanty = function (ind) {
            if ($scope.obj.ItemList[ind].CHECK === 'YES') {
                if ($scope.obj.ItemList[ind].ISSU_QTY===0) {
                    $scope.obj.ItemList[ind].ISSU_QTY += 1;
                } else {
                    if ($scope.obj.ItemList[ind].STOCK > $scope.obj.ItemList[ind].ISSU_QTY) {
                        $scope.obj.ItemList[ind].ISSU_QTY = $scope.obj.ItemList[ind].ISSU_QTY;
                    } else {
                        //alert('Stock quantity is less than your Issue quantity');
                        alert($scope.getSingleMsg('S0036'));
                    }
                }
            ...
            if (btn === 'plus') {
                if ($scope.obj.ItemList[i].CHECK === 'YES') {
                    if ($scope.obj.ItemList[i].ISSU_QTY === 0) {
                        ...
                        if ($scope.obj.ItemList[i].STOCK > $scope.obj.ItemList[i].ISSU_QTY) {
                            $scope.obj.ItemList[i].ISSU_QTY += 1; 
                        } else {
                            //alert('Stock quantity is less than your Issue quantity');
                            alert($scope.getSingleMsg('S0036'));
                    ...
                    if ($scope.obj.ItemList[i].ISSU_QTY === 0) {
                        ...
                        if ($scope.obj.ItemList[i].STOCK > $scope.obj.ItemList[i].ISSU_QTY) {
                            $scope.obj.ItemList[i].ISSU_QTY += $scope.obj.ItemList[i].ISSU_QTY;
                            $scope.obj.ItemList[i].CHECK = 'YES';
                        } else {
                            //alert('Stock quantity is less than your Issue quantity');
                            alert($scope.getSingleMsg('S0036'));
